 ...
 * Time: 9:18
 */
    define('DISCOUNT_BEFORE_SEVEN_CLOCK', 0.7);
    ...
    function discount($basePrice, $sum, $beforeSevenClock = false){
        if ($beforeSevenClock){
...
if($basePrice != ''){
    $sum = 0;
    $day = 1;
    $beforeSevenClock = true;
    while($day <= 31){
        $sum += discount($basePrice, $sum, $beforeSevenClock);
        $sum += discount($basePrice, $sum);
        echo "{'day':$day,'price':$sum,'beforeSevenClock':'true'},\n";
        $day++;
        //break;
    }
    ...
    $sum = 0;
    $day = 1;
    $beforeSevenClock = false;
    while($day <= 31){
        $sum += discount($basePrice, $sum, $beforeSevenClock);
        $sum += discount($basePrice, $sum);
        echo "{'day':$day,'price':$sum,'beforeSevenClock':'false'},\n";
        $day++;
        //break;
    }
}else{
...
if ($basePrice != ''){
?>
	chart.line().position('day*price').color('beforeSevenClock');
<?php 
}else{
?>
