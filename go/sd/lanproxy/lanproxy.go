...
func LanProxyMain(clientKey, serverHost string, serverPort int, enableSSL bool, certPath string) {
	...
	log.Println("client key:", clientKey)
	log.Println("server addr:", serverHost)
	log.Println("server port:", serverPort)
	log.Println("enable ssl:", enableSSL)
	cerPath := certPath
	if cerPath == "" {
	...
	start(clientKey, serverHost, serverPort, conf)
}
