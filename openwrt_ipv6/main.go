...
func main() {
	...
	addr4, err := net.ResolveUDPAddr("udp4", PORT4)
	if err != nil {
		log.Fatalf("Error resolving UDP address: %v", err)
	}
	...
	fmt.Printf("Listening on UDP port %s (IPv4 only)\n", PORT4)
	...
	for {
		...
		// 读取 UDP 数据
		n, clientAddr, err = conn4.ReadFromUDP(buffer)
		if err != nil {
			log.Printf("Error reading UDP message: %v", err)
			continue
		}
		...
		// 将数据写入文件（覆盖模式）
		err = os.WriteFile(FILE4, []byte(message), 0644)
		if err != nil {
			log.Printf("Error writing to file: %v", err)
			continue
		}
